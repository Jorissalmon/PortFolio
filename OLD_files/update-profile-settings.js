...
async function updateProfileSettings() {
    console.log('🚀 Mise à jour du Profile Settings dans Contentful...\n');
    ...
    try {
        ...
        if (entries.total > 0) {
            console.log(`📝 Mise à jour de l'entrée existante (ID: ${entries.items[0].sys.id})...\n`);
            entry = entries.items[0];

            // Mettre à jour les champs
            entry.fields.title = { 'en-US': profileSettings.title };
            entry.fields.chatbotPrompt = { 'en-US': profileSettings.chatbotPrompt };
            entry.fields.aboutDescription = { 'en-US': profileSettings.aboutDescription };
            entry.fields.cvLink = { 'en-US': profileSettings.cvLink };
            entry.fields.location = { 'en-US': profileSettings.location };
            entry.fields.phone = { 'en-US': profileSettings.phone };
            entry.fields.email = { 'en-US': profileSettings.email };
            ...
            entry = await environment.createEntry('profileSettings', { fields });
        }

        // Publier l'entrée
        const publishedEntry = await entry.publish();

        console.log('✅ Profile Settings mis à jour et publié avec succès!');
