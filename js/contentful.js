/**
 * contentful.js - Solution simplifiée sans encadrés de chargement
 */
...
document.head.insertAdjacentHTML(
  ...
  (function() {
    if (typeof window.CONFIG === "undefined" || !window.CONFIG.CONTENTFUL) {
      console.error("Configuration Contentful manquante! Utilisez le fichier config.js");
  ...
  // Version sécurisée du service Contentful qui utilise config.js
  window.contentfulService = {
    // Utiliser les clés depuis le fichier de configuration
    get SPACE_ID() {
      return window.CONFIG?.CONTENTFUL?.SPACE_ID || "";
    },
  
    get ACCESS_TOKEN() {
      return window.CONFIG?.CONTENTFUL?.ACCESS_TOKEN || "";
    },
    ...
    isConfigValid: function() {
      const valid = !!(this.SPACE_ID && this.ACCESS_TOKEN);
      if (!valid) {
        console.error("Configuration Contentful manquante. Vérifiez le fichier config.js");
      }
      return valid;
    },
    ...
    getAssetById: async function(assetId) {
      if (!this.isConfigValid()) {
        throw new Error('Configuration Contentful invalide');
      }
      ...
      try {
        // URL correcte pour récupérer un asset
        const apiUrl = `https://cdn.contentful.com/spaces/${this.SPACE_ID}/environments/master/assets/${assetId}?access_token=${this.ACCESS_TOKEN}`;
        
        console.log(`Récupération de l'asset ${assetId} via API directe`);
        
        const response = await fetch(apiUrl);
        
        if (!response.ok) {
          throw new Error(`Erreur lors de la récupération de l'asset: ${response.status}`);
        }
            ...
            try {
              ...
              if (node.data && node.data.target) {
                ...
                if (node.data.target.sys && node.data.target.sys.id) {
                  ...
                  // Si c'est un tableau (comme vous le mentionnez "en list")
    ...
    getArticleById: async function(id) {
      if (!this.isConfigValid()) {
        console.error("Configuration invalide pour getArticleById");
        return null;
      }
  
      try {
        // Construire l'URL de l'API pour un article spécifique
        const apiUrl = `https://cdn.contentful.com/spaces/${this.SPACE_ID}/environments/master/entries/${id}?access_token=${this.ACCESS_TOKEN}&include=10`;
  
        console.log("Récupération de l'article avec URL:", apiUrl);
    ...
    getArticles: async function() {
      if (!this.isConfigValid()) {
        console.error('Configuration invalide pour getArticles');
        return [];
      }
  
      try {
        // Construire l'URL de l'API Contentful
        const apiUrl = `https://cdn.contentful.com/spaces/${this.SPACE_ID}/environments/master/entries?access_token=${this.ACCESS_TOKEN}&content_type=blogPortfolio&include=2`;
    ...
    getProjects: async function() {
      if (!this.isConfigValid()) {
        console.error('Configuration invalide pour getProjects');
        return [];
      }
  
      try {
        // Construire l'URL de l'API Contentful pour les projets
        const apiUrl = `https://cdn.contentful.com/spaces/${this.SPACE_ID}/environments/master/entries?access_token=${this.ACCESS_TOKEN}&content_type=projectPortfolio&include=2`;
...
window.contentfulService.getProjectById = async function(projectId) {
  if (!this.isConfigValid()) {
    console.error('Configuration invalide pour getProjectById');
    return null;
  }

  try {
    // Construire l'URL de l'API pour un projet spécifique
    const apiUrl = `https://cdn.contentful.com/spaces/${this.SPACE_ID}/environments/master/entries/${projectId}?access_token=${this.ACCESS_TOKEN}&include=10`;

    console.log("Récupération du projet avec URL:", apiUrl);

    // Appeler l'API
    const response = await fetch(apiUrl);

    if (!response.ok) {
      throw new Error(`Projet non trouvé (${response.status})`);
    }
