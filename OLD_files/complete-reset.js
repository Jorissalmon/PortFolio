...
async function completeReset() {
    ...
    log('🔄 Reset complet et recréation de profileSettings...\n');
    ...
    try {
        ...
        // ÉTAPE 1: Supprimer toutes les entrées
        log('🗑️  Étape 1: Suppression des entrées...');
        try {
            ...
            for (const entry of entries.items) {
                try {
                    if (entry.sys.publishedVersion) {
                        ...
                        log(`   - Dépublié: ${entry.sys.id}`);
                    }
                    await entry.delete();
                    log(`   - Supprimé: ${entry.sys.id}`);
                } catch (e) {
                    log(`   ⚠️  Erreur sur entrée ${entry.sys.id}: ${e.message}`);
                }
            }
            log('✅ Toutes les entrées supprimées\n');
        } catch (error) {
            log('ℹ️  Aucune entrée à supprimer\n');
        }

        // ÉTAPE 2: Supprimer le content type
        log('🗑️  Étape 2: Suppression du content type...');
        try {
            const contentType = await environment.getContentType('profileSettings');

            if (contentType.sys.publishedVersion) {
                await contentType.unpublish();
                log('   - Content type dépublié');
            }

            await contentType.delete();
            log('✅ Content type supprimé\n');
        } catch (error) {
            log('ℹ️  Aucun content type à supprimer\n');
        }

        // ÉTAPE 3: Recréer le content type avec aboutDescription comme Text simple
        log('📐 Étape 3: Création du nouveau content type...');

        const newContentType = await environment.createContentTypeWithId('profileSettings', {
            ...
            fields: [
                ...
                },
                {
                    ...
                    type: 'Text',  // ← TEXT SIMPLE, pas Array
                    required: false
                },
                {
                    id: 'cvLink',
        ...
        log('✅ Content type créé et publié\n');

        // ÉTAPE 4: Insérer les données
        log('📝 Étape 4: Insertion des données...');
        ...
        log('\n🎉 SUCCÈS COMPLET !');
        log('✅ ProfileSettings créé et publié');
