...
document.head.insertAdjacentHTML(
  ...
  `
  <style>
    /* Style du bouton amélioré - Adapted for Glass Theme */
    ...
    .article-nav .btn-primary:hover {
      background: linear-gradient(135deg, #bc13fe, #6e00ff);
...
window.contentfulService = {
  ...
  renderRichTextWithIds: function (richTextContent) {
    ...
    function renderNode(node) {
      ...
      switch (node.nodeType) {
        ...
        case "hyperlink":
          return `<a href="${node.data.uri}" target="_blank" rel="noopener noreferrer">${node.content ? node.content.map(renderNode).join("") : ""
            }</a>`;
        ...
        case "blockquote":
          return `<blockquote>${node.content ? node.content.map(renderNode).join("") : ""
            }</blockquote>`;
  ...
  /**
   * Récupère la configuration du profil (prompt IA, job titles, description, CV, etc.)
   * @returns {Promise<Object>} Configuration du profil
   */
  getProfileSettings: async function () {
    try {
      console.log('Récupération de la configuration du profil via API Vercel');
      ...
      if (!data.items || data.items.length === 0) {
        console.log('Aucune configuration de profil trouvée');
        return null;
      }

      console.log('Configuration du profil récupérée:', data.items.length);
      return data;
    } catch (error) {
  ...
  /**
   * Récupère les compétences depuis Contentful
   * @returns {Promise<Object>} Liste des compétences
   */
  getSkills: async function () {
    try {
      console.log('Récupération des compétences via API Vercel');
      ...
      console.log('Compétences récupérées:', data.items.length);
      return data;
    } catch (error) {
  ...
  /**
   * Récupère les formations depuis Contentful
   * @returns {Promise<Object>} Liste des formations
   */
  getEducation: async function () {
    try {
      console.log('Récupération des formations via API Vercel');
      ...
      processedData.items = await Promise.all(data.items.map(async (item) => {
        const fields = { ...item.fields };

        // Récupérer l'URL du logo si présent
        if (fields.logo && fields.logo.sys && fields.logo.sys.id) {
          const logoId = fields.logo.sys.id;

          if (data.includes && data.includes.Asset) {
            const asset = data.includes.Asset.find(a => a.sys.id === logoId);
            if (asset && asset.fields && asset.fields.file) {
              fields.logo = `https:${asset.fields.file.url}`;
            }
          }
        }
      ...
      console.log('Formations récupérées:', processedData.items.length);
  ...
  /**
   * Récupère les expériences professionnelles depuis Contentful
   * @returns {Promise<Object>} Liste des expériences
   */
  getExperiences: async function () {
    try {
      console.log('Récupération des expériences via API Vercel');
      ...
      console.log('Expériences récupérées:', processedData.items.length);
      return processedData;
    } catch (error) {
  ...
  /**
   * Récupère les passions depuis Contentful
   * @returns {Promise<Object>} Liste des passions
   */
  getPassions: async function () {
    try {
      console.log('Récupération des passions via API Vercel');

      const data = await this.callContentfulApi('entries', null, {
        content_type: 'passion',
        include: 2,
        order: 'fields.order'
      });

      if (!data.items || data.items.length === 0) {
        console.log('Aucune passion trouvée');
        return null;
      }
      ...
      processedData.items = await Promise.all(data.items.map(async (item) => {
        ...
        if (fields.image && fields.image.sys && fields.image.sys.id) {
          ...
          if (data.includes && data.includes.Asset) {
            ...
            if (asset && asset.fields && asset.fields.file) {
              fields.image = `https:${asset.fields.file.url}`;
            }
          }
        }
      ...
      console.log('Passions récupérées:', processedData.items.length);
      return processedData;
    } catch (error) {
      console.error("Erreur lors de la récupération des passions:", error);
      return null;
    }
  }
};
